...
class Main {
    ...
    public static String[] calc(String inputString) {
        ...
        var operation = (int) inputString.codePoints().filter(ch -> ch == '+' || ch == '-' || ch == '*' || ch == '/').count();
//       Подсчёт кол-ва операндов, не более 1-го в строке
        if (operation > 1 || operation == 0) {
            Scanner inputString_a_value_again = new Scanner(System.in);
            System.out.println("Неверный формат ввода данных. Введите один операнд: ");
    ...
    public static boolean its_an_arabic_numbers = true;
...
class Romes extends Number {
    ...
    private String convert_result_to_Romes(int n, int ostatok) {
        ...
        //Обработка отрицательных римских чисел
        if (n > 0) {
            n = n - 10;
    ...
    public void sub() {
        ...
        if (result_int <= 0) {
            try {
                ...
                System.out.println("В римской системе нет отрицателных чисел или нуля.");
                return;
            }
        } else {
            result_string = convert_result_to_Romes(result_int, result_int);
